...
// Builds an in-page expression for find-on-page. Uses Chromium's
// non-standard window.find(), which advances from the current selection and
// sets the new match as the selection. While find is active we restyle
// ::selection (yellow on black) so the match is clearly visible — the style
// is removed when the find bar closes (see findStop).
function findOnPageScript(query: string, backward: boolean, fromStart: boolean): string {
  return `(() => {
    ...
    if (!document.getElementById('__bridge-find-style')) {
      ...
      st.textContent = '::selection { background-color: #ffd33d !important; color: #000 !important; }';
      document.head.appendChild(st);
    }
    // window.find args: searchString, caseSensitive, backwards, wrapAround,
    // wholeWord, searchInFrames, showDialog.
    return window.find(query, false, backward, true, false, false, false);
  })()`;
}

// Removes the find-mode ::selection style and clears the lingering match
// selection so closing the find bar leaves the page in a clean state.
